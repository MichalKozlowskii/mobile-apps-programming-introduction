            ...
            for (i in 0..3) {
                Row(
                    modifier = Modifier
                        ...
                        .weight(0.25f), // Adjust the weight as needed
                    verticalAlignment = Alignment.CenterVertically, // Center elements vertically
                    horizontalArrangement = Arrangement.Start // Align elements horizontally (to the start)
                ) {
                    RadioButton(
                        selected = (selectedAnswerIndex == i),
                        onClick = { onAnswerSelected(i) },
                        modifier = Modifier.padding(end = 8.dp) // Adds spacing between radio button and text
                    )
                    Text(
